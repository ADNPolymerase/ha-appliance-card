...
- Normalisation d'état : traduit n'importe quelle chaîne brute renvoyée par ton intégration (`Idle`, `Running`, `RUNNING`, `wash`, ...) vers un vocabulaire commun en veille / en cours / en pause / terminé / différé / erreur, avec une détection automatique par mots-clés et une surcharge explicite optionnelle via `state_map`.
